...
public class Board {
	...
	String getLetters(int n) {
		...
		int limit = Math.min(n, this.letters.size());
		for (int i = 0; i < limit; i++)
			r.append(this.letters.remove(0).getLetter());
		return r.toString();
	}
	
	public boolean anyLettersLeft() {
	...
	public ResultadoJugada movement(List<JSONObject> jugada) {
		...
		// } else {
		// // Paso a minuscula para que coincida con la BBDD
		// List<Palabra>
		// palabras=this.palabrasRepo.findByTexto(cadena.getText().toLowerCase());
		// if (palabras.isEmpty())
		...
		if (resultado.acceptsAll()) {
			...
			String letrasNuevas = this.getLetters(Math.min(letters.size(), jugada.size()));
	...
	/**
	 * Se ha separado la validacion por el error que ocurre en la primera jugada en
	 * la cual si se introduce una palabra erronea, se cambia la variable
	 * centroOcupado y ocasiona errores en jugadas siguientes
	 * 
	 * @param resultado
	 * @param cadenas
	 */
	private void checkValid(ResultadoJugada resultado, List<Cadena> cadenas) {
		for (Cadena cadena : cadenas) {
			if (cadena.length() == 1) {
				resultado.addAccepted(cadena);
			} else {
				// Paso a minuscula para que coincida con la BBDD
				List<Palabra> palabras = this.palabrasRepo.findByTexto(cadena.getText().toLowerCase());
				if (palabras.isEmpty())
					resultado.addNotAccepted(cadena);
				else
					resultado.addAccepted(cadena);
	...
	private void comprobarAdyacencia(List<JSONObject> jugada) throws Exception {
		...
		throw new Exception("Fichas mal posicionadas (ComprobarAdyacencia)");
	}
	...
	/**
	 * Si la jugada es invalida quitamos lo que habiamos puesto
	 * 
	 * @param jugada
	 * @throws Exception
	 */
	public void quitarLetras(List<JSONObject> jugada) throws Exception {
		int row, col;
		for (int i = 0; i < jugada.size(); i++) {
			JSONObject casilla = jugada.get(i);
			row = casilla.getInt("row");
			...
			if (!this.squares[row][col].isEmpty()) {
				...
				this.squares[row][col].setProvisional(false);
			}
		}
	}
	...
	private boolean comprobarUnSoloSentido(List<JSONObject> jugada) throws Exception {
		...
		if (enHorizontal && enVertical)
			throw new Exception("Fichas mal posicionadas (ComprobarUnSoloSentido)");
		if (enHorizontal)
	...
	public String toString() {
		...
		for (int i = 0; i < squares.length; i++) {
			for (int j = 0; j < squares[i].length; j++) {
				if (this.squares[i][j].getLetter() == '\0') {
					boardString += '-';
				} else {
					boardString += this.squares[i][j].getLetter();
				}
				// boardString += this.squares[i][j].getLetter();
			}
		}
