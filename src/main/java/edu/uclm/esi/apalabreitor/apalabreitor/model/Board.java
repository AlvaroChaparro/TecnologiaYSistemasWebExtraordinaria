...
//		}
	...
	/**
	 * Se ha separado la validacion por el error que ocurre en la primera
	 * jugada en la cual si se introduce una palabra erronea, se cambia
	 * la variable centroOcupado y ocasiona errores en jugadas siguientes
	 * @param resultado
	 * @param cadenas
	 * @return
	 */
	private List<Cadena> checkValid(ResultadoJugada resultado, List<Cadena> cadenas) {
		for (Cadena cadena : cadenas) {
			if (cadena.length()==1) {
				resultado.addAccepted(cadena);
			} else {
